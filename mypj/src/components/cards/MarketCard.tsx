...
export default function MarketCard({ item, isLoading }: MarketCardProps) {
  ...
  const priceStr = isKRW
    ...
    : formatPrice(item.price, item.price < 100 ? 2 : 2)
  ...
  return (
    <div className="dc p-4">
      ...
      <div className="num text-2xl font-bold text-[#c9d8f5] mb-0.5 leading-none">
        {priceStr}
        {item.currency && <span className="num text-xs font-normal text-[#3a506e] ml-1.5">{item.currency}</span>}
      </div>
